...
export default function ReceiptPage({ params }: { params: Promise<{ receiptId: string }> }) {
  ...
  const receipt = {
    id: receiptId || "RCP-20260315-0819",
    date: "March 15, 2026 · 14:23 WAT",
    storeName: "Amara's Fashion",
    storeAddress: "Victoria Island, Lagos",
    customer: "Fatima Bello",
    items: [
      { name: "Owanbe Lace Gown", qty: 1, price: 120 },
      { name: "Ankara Midi Skirt", qty: 1, price: 40 },
    ],
    subtotal: 160.00,
    fee: 0.00,
    total: 160.00,
       ...
       >
                ...
                <div className="space-y-3">
                   <div className="flex justify-between items-center bg-black/40 p-3 rounded-lg">
                      ...
                      <div className="flex items-center gap-2">
                         ...
                         <button className="text-muted-foreground hover:text-white transition-colors" title="Copy Hash"><Copy className="w-3 h-3" /></button>
                         <button className="text-muted-foreground hover:text-white transition-colors" title="View on Explorer"><ExternalLink className="w-3 h-3" /></button>
                      </div>
                   </div>
          ...
          <div className="px-8 pb-8 lg:px-12 lg:pb-12 flex flex-col gap-4">
             <div className="flex flex-col sm:flex-row gap-4 items-center">
                <Button variant="outline" className="flex-1 w-full rounded-2xl h-12 border-white/10 hover:bg-white/5 text-xs font-bold uppercase tracking-widest text-white gap-2">
                   <Printer className="w-4 h-4" /> Print Receipt
                </Button>
                <Button className="flex-1 w-full rounded-2xl h-12 bg-white/5 hover:bg-white/10 text-white font-bold text-xs uppercase tracking-widest gap-2 border border-white/10">
